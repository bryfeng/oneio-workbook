...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${document.title}</title><meta name="description" content="Static API examples for ONE business wallets, Fireblocks Flow and crypto batch payouts, with an LLM handoff."><style>${css}\n${pageCss}</style></head>
<body><a class="skip-link" href="#business-wallet">Skip to walkthrough</a><main class="walkthrough-document">
<header class="masthead"><div><div class="eyebrow">CLEARER / ONE</div><h1>${document.title}</h1><p class="subtitle">${escape(document.subtitle)}</p></div><div class="edition"><span>Static discussion draft</span><a href="./">API brief ↗</a><a href="system.html">System map ↗</a><a href="one-api-walkthrough.md" download>Download LLM brief ↓</a></div></header>
...
<figure class="journey-strip" aria-label="Example: wallet setup, one thousand USDC received, eight hundred USDC paid"><div><small>01 / SET UP</small><strong>Business wallet</strong><span>USDC on Base · one merchant</span></div><span class="journey-arrow" aria-hidden="true">→</span><div><small>02 / COLLECT</small><strong>1,000 USDC received</strong><span>Flow → the same wallet</span></div><span class="journey-arrow" aria-hidden="true">→</span><div><small>03 / PAY OUT</small><strong>800 USDC to suppliers</strong><span>400 + 250 + 150 · 200 retained</span></div></figure>
...
<footer><span>clearer.money · Example data throughout · Auto-conversion on hold</span><span>Provider docs checked ${document.date}</span></footer>
  ...
  await writeFile(path.join(dist,'one-api-walkthrough.html'),markup.replace('href="./"','href="one-gateway-api-brief.html"').replace('href="system.html"','href="one-system-map.html"'));
